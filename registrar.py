...
def registrarYalmacenar(ms, tipoCab):
    ...
    print(f"Start = {start} \r")                       # y salga del while con el gas off  
    i=0
            ...
            try:
                while raw_frame[0] == 240 and raw_frame[1] == 240:                  # 240 = 0xF0: Datos / 250 = 0xFA: Tiempo
                    if port.in_waiting >= 11:               # Verifica si hay suficiente data en el buffer
                        ...
                        print(",".join(map(str, raw_frame)))   
                                    
                        datos = raw_frame[2:11]           
                        #print(",".join(map(str, datos)))
                            
                        n = datos[0]*256+datos[1]

                        print(f"registrarYAlmacenar - nro. trama ={n} \r")
                        ...
                        if checksum0 == checksum1:
                            ...
                            if len(buffer) >= tamanio_buffer:
                                print("Guardando buffer en archivo \r")
                                with open('data/'+fname, fmode, newline="") as file:
                            ...
                            logger.error(" Error en checksum, se desecha trama \r")
                            
                    else:
                        no_hay_mas_tramas += 1
                        if no_hay_mas_tramas >= 100000:
                            print("No hay más tramas")
                            start = 0
                            break


            except Exception as e:
                print(f"while error...{e}\r")
                pass
    print("Finaliza registrar.py \r")
    return fname
